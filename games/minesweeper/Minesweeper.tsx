...
export default function Game() {
    ...
    const [flagged, setFlagged] = useState(new Array(settings.mines).fill(false));
    const [revealedCells, setRevealedCells] = useState(new Array(settings.rows * settings.cols).fill(false));
    ...
    const [board, setBoard] = useState(initialize(settings));
    const [display, setDisplay] = useState(getDisplayValues(board));
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    width: "100vh",
                    height: "auto",
                    gridTemplateRows: `repeat(${settings.rows}, calc(100vmin / ${Math.max(settings.rows, settings.cols)})) `,
                    gridTemplateColumns: `repeat(${settings.cols}, calc(100vmin / ${Math.max(settings.rows, settings.cols)}))`,
                    overflow: "hidden",
                    margin: "auto",
                }}
            >
